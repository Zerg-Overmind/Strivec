...
class DenseGrid(nn.Module):
    def __init__(self, channels, world_size, xyz_min, xyz_max, **kwargs):
        ...
        self.register_buffer('xyz_min', torch.Tensor(xyz_min) if not torch.is_tensor(xyz_min) else xyz_min)
        self.register_buffer('xyz_max', torch.Tensor(xyz_max) if not torch.is_tensor(xyz_max) else xyz_max)
        self.grid = nn.Parameter(torch.zeros([1, channels, *world_size], device="cuda"))
