...
def create_optimizer_or_freeze_model(model, args, global_step):
    decay_steps = args.pre_lrate_decay * 1000
    decay_factor = 0.1 ** (global_step/decay_steps)

    param_group = []
    for k in args.__dict__.keys():
        if not k.startswith('pre_lrate_'):
            continue
        k = k[len('pre_lrate_'):]
        ...
        lr = getattr(args, f'pre_lrate_{k}') * decay_factor
        if lr > 0:
            print(f'create_optimizer_or_freeze_model: param {k} lr {lr}')
            if isinstance(param, nn.Module):
                param = param.parameters()
            param_group.append({'params': param, 'lr': lr, 'skip_zero_grad': args.skip_zero_grad})
        else:
