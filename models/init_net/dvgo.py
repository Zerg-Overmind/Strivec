...
class DirectVoxGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
                 ...
                 fast_color_thres=0, density_config={}, rgbnet_dim=0, rgbnet_direct=True, rgbnet_full_implicit=False,
        ...
        self.density = grid.DenseGrid(channels=1, world_size=self.world_size,
        ...
        self.k0_dim = 3
        self.k0 = grid.DenseGrid(channels=self.k0_dim, world_size=self.world_size,
            xyz_min=self.xyz_min, xyz_max=self.xyz_max)
        self.rgbnet = None
    ...
    def update_occupancy_cache(self):
        cache_grid_xyz = torch.stack(torch.meshgrid(
            torch.linspace(self.xyz_min[0], self.xyz_max[0], self.mask_cache.mask.shape[0], device="cuda"),
            torch.linspace(self.xyz_min[1], self.xyz_max[1], self.mask_cache.mask.shape[1], device="cuda"),
            torch.linspace(self.xyz_min[2], self.xyz_max[2], self.mask_cache.mask.shape[2], device="cuda")), -1)
    ...
    def voxel_count_views(self, rays_o_tr, rays_d_tr, imsz, near, far, stepsize, downrate=1, irregular_shape=False):
        ...
        rng = torch.arange(N_samples, device=rays_o_tr.device)[None].float()
