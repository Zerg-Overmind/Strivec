...
from opt_dbasis import config_parser
args = config_parser()
print(args)
...
def reconstruction(args, geo, test_dataset, train_dataset):
    ...
    if args.ckpt is not None:
        ...
        tensorf = eval(args.model_name)(aabb, None, device,
            ...
            fea2denseAct=args.fea2denseAct, local_dims=args.local_dims_init, pnts=geo, args=args)
    ...
    if args.local_dims_trend is not None:
        ...
        for i in range(len(args.local_dims_trend)):
            ...
            for j in range(len(args.local_dims_init[i])):
                level_dim_lst.append(torch.round(trend * args.local_dims_final[i][j] / args.local_dims_final[i][0]).long())
