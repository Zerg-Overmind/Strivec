...
class PointTensorBase(TensorBase):
    ...
    def filtering_rays(self, all_rays, all_rgbs, N_samples=-1, chunk=10240 * 1, cvrg=True, bbox_only=False, apply_filter=True):
        ...
        for idx_chunk in func(idx_chunks):#img_list:#
            ...
            if cvrg:
                mask_inrange = filter_ray_by_cvrg(xyz_sampled, xyz_inbbox, self.units if self.args.tensoRF_shape == "cube" else self.units_3, self.aabb[0], self.aabb[1], (self.tensoRF_cvrg_inds >= 0).contiguous())
    ...
    def sample_2_tensoRF_cvrg(self, xyz_sampled, pnt_rmatrix=None, rotgrad=False):
        ...
        if self.args.tensoRF_shape == "cube":
            if self.args.rot_init is not None and not rotgrad:
                mask, local_gindx_s, local_gindx_l, local_gweight_s, local_gweight_l, local_kernel_dist, tensoRF_id, agg_id = search_geo_cuda.sample_2_rot_tensoRF_cvrg(xyz_sampled.contiguous(), self.aabb[0], self.aabb[1], self.units, self.local_range, self.local_dims[:3], self.tensoRF_cvrg_inds, self.tensoRF_count, self.tensoRF_topindx, pnt_rmatrix, self.pnt_xyz, self.K_tensoRF, self.KNN)
            elif self.args.rot_init is not None and rotgrad:
                mask, dist, local_kernel_dist, tensoRF_id, agg_id = search_geo_cuda.sample_2_rotdist_tensoRF_cvrg(xyz_sampled.contiguous(), self.aabb[0], self.aabb[1], self.units, self.local_range, self.local_dims[:3], self.tensoRF_cvrg_inds, self.tensoRF_count, self.tensoRF_topindx, pnt_rmatrix, self.pnt_xyz, self.K_tensoRF, self.KNN)
                ...
                local_gindx_s, local_gindx_l, local_gweight_s, local_gweight_l, local_kernel_dist, tensoRF_id, agg_id = search_geo_cuda.sample_2_tensoRF_cvrg(xyz_sampled.contiguous(), self.aabb[0], self.aabb[1], self.units, self.local_range, self.local_dims[:3], self.tensoRF_cvrg_inds, self.tensoRF_count, self.tensoRF_topindx, self.geo_xyz_recenter, self.pnt_xyz, self.K_tensoRF, self.KNN)

        elif self.args.tensoRF_shape == "sphere":
            mask, local_gindx_s, local_gindx_l, local_gweight_s, local_gweight_l, local_kernel_dist, tensoRF_id, agg_id = search_geo_cuda.sample_2_sphere_tensoRF_cvrg(xyz_sampled.contiguous(), self.aabb[0], self.aabb[1], self.units, self.radiusl, self.radiush, self.local_dims[:3], self.tensoRF_cvrg_inds, self.tensoRF_count, self.tensoRF_topindx, self.pnt_xyz, self.K_tensoRF, self.KNN)
        if mask is not None:
...
class PointTensorCP(PointTensorBase):
    ...
    def init_one_svd(self, n_component, local_dims, scale, device):
        ...
        for i in range(len(self.vecMode)):
            ...
            line_coef.append(torch.nn.Parameter(scale * torch.randn((len(self.geo), n_component, local_dims[vec_id] + 1))))
