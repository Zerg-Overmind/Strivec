...
class GaussianMixture(torch.nn.Module):
    ...
    """
    def __init__(self, n_components, n_features, covariance_type="full", max_iter=100, tol=1e-3, eps=1.e-6, init_params="kmeans", mu_init=None, var_init=None, device="cuda"):
        """
        ...
        self.device = device
        self._init_params(device=device)
    ...
    def _init_params(self, device="cuda"):
        if self.mu_init is not None:
            ...
            # (1, k, d)
            self.mu = torch.nn.Parameter(self.mu_init, requires_grad=False, device=device)
        else:
            self.mu = torch.nn.Parameter(torch.randn(1, self.n_components, self.n_features, device=device), requires_grad=False)
        ...
        if self.covariance_type == "diag":
            if self.var_init is not None:
                ...
                self.var = torch.nn.Parameter(self.var_init, requires_grad=False, device=device)
            else:
                self.var = torch.nn.Parameter(torch.ones(1, self.n_components, self.n_features, device=device), requires_grad=False)
            ...
            if self.var_init is not None:
                ...
                self.var = torch.nn.Parameter(
                    torch.eye(self.n_features, device=device).reshape(1, 1, self.n_features, self.n_features).repeat(1, self.n_components, 1, 1),
                    requires_grad=False
                )

        # (1, k, 1)
        self.pi = torch.nn.Parameter(torch.full((1, self.n_components, 1), (1. / self.n_components), device=device), requires_grad=False)
        self.params_fitted = False

    @property
    def covariances_(self):
        return torch.squeeze(self.var).cpu().numpy()

    @property
    def means_(self):
        return torch.squeeze(self.mu).cpu().numpy()
