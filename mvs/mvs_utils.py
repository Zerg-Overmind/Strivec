...
# def construct_voxrange_points_mean(pnts, vox_range, space_min=None, space_max=None, vox_center=False):
#     # xyz, N, 3
#
#     if space_min is None: # 1
#         xyz_min, xyz_max = torch.min(pnts[..., :3], dim=-2)[0], torch.max(pnts[..., :3], dim=-2)[0]
...
#         space_edge = space_max - space_min
#         mask = (pnts[..., :3] - space_min[None,...])
#         mask *= (space_max[None,...] - pnts[..., :3])
#         mask = torch.prod(mask, dim=-1) > 0
#         pnts = pnts[mask, :]
#
...
#     shift_range = torch.as_tensor([vox_range[0].cpu().item() / 2 if odd_even[0] % 2 ==1 else 0, vox_range[1].cpu().item() / 2 if odd_even[1] % 2 ==1 else 0, vox_range[2].cpu().item() / 2 if odd_even[2] % 2 ==1 else 0], device = pnts.device, dtype=pnts.dtype)
#     shift = (xyz_mid - shift_range)[None, ...]  # shift = space_min[None, ...]
#     print("shift_range", shift_range)
#     print("shift", shift)
#     xyz_shift = pnts[..., :3] - shift
...
#         geo = vox_center
#     else:
#         geo = scatter_mean(pnts[..., :3], inv_idx, dim=0)
#
#     return geo, pnts, sparse_grid_idx, inv_idx
...
def construct_voxrange_points_mean(pnts, vox_range, space_min=None, space_max=None, vox_center=False):
    ...
    shift = space_min
    xyz_shift = pnts[..., :3] - shift
    sparse_grid_idx, inv_idx = torch.unique(torch.floor(xyz_shift / vox_range[None, ...]).to(torch.int32), dim=0,
                                            return_inverse=True)
    if vox_center:  # 1
        vox_center = (sparse_grid_idx + 0.5) * vox_range[None, ...] + shift
        # geo = torch.cat([vox_center, scatter_mean(geo[..., -1], inv_idx, dim=0)[..., None]], dim=-1)
...
def alpha_masking(points, alphas, intrinsics, c2ws, w2cs, near_far, args=None):
    ...
    for i in range(len(alphas)):
        ...
        del cam_xyz
        torch.cuda.empty_cache()
        if args is not None and args.inall_img == 0:
...
def volume_expand(occ, grid_expsize):
    vol = torch.clone(occ)
    xdim, ydim, zdim = occ.shape
    occ = torch.nn.functional.pad(occ, (grid_expsize, grid_expsize, grid_expsize, grid_expsize, grid_expsize, grid_expsize), mode='constant', value=0)
    for i in range(-grid_expsize, grid_expsize+1):
        for j in range(-grid_expsize, grid_expsize + 1):
            for k in range(-grid_expsize, grid_expsize + 1):
                if i == 0 and j == 0 and k == 0:
                    continue
                else:
                    vol += occ[i+grid_expsize:i+grid_expsize+xdim, j+grid_expsize:j+grid_expsize+ydim, k+grid_expsize:k+grid_expsize+zdim]
    return (vol > 0).to(torch.int32)

def construct_grid_points(xyz_val, opt, partition_xyz=None):

    xyz = xyz_val if partition_xyz is None else partition_xyz
    if opt.ranges[0] < -90.0:
        xyz_min, xyz_max = torch.min(xyz, dim=-2)[0], torch.max(xyz, dim=-2)[0]
        space_edge = torch.max(xyz_max - xyz_min) * 1.03
        xyz_mid = (xyz_max + xyz_min) / 2
        space_min = xyz_mid - space_edge / 2
        space_max = xyz_mid + space_edge / 2
    else:
        space_min, space_max = torch.as_tensor(opt.ranges[:3], device=xyz.device, dtype=torch.float32),  torch.as_tensor(opt.ranges[3:], device=xyz.device, dtype=torch.float32)
        space_edge = space_max - space_min
        mask = (xyz_val - space_min[None, ...])
        mask *= (space_max[None, ...] - xyz_val)
        mask = torch.prod(mask, dim=-1) > 0
        xyz = xyz_val[mask, :]
    grid_res = torch.ceil(space_edge / torch.as_tensor(opt.grid_vx_sz, device=space_edge.device, dtype=torch.float32)).to(torch.int32)
    xyz_shift = xyz - space_min[None, ...]
    construct_vox_idx = torch.floor(xyz_shift / opt.grid_vx_sz).to(torch.int32).long()
    full_vox_occ = torch.zeros([grid_res[0], grid_res[1], grid_res[2]], device=space_edge.device, dtype=torch.int32)
    full_vox_occ[construct_vox_idx[..., 0], construct_vox_idx[..., 1], construct_vox_idx[..., 2]] = 1
    full_vox_occ = volume_expand(full_vox_occ, opt.grid_expsize)
    full_grid_idx = torch.zeros([grid_res[0]+1, grid_res[1]+1, grid_res[2]+1], device=space_edge.device, dtype=torch.int32)
    full_grid_idx[:-1, :-1, :-1] += full_vox_occ
    full_grid_idx[:-1, 1:, :-1] += full_vox_occ
    full_grid_idx[:-1, :-1, 1:] += full_vox_occ
    full_grid_idx[:-1, 1:, 1:] += full_vox_occ
    full_grid_idx[1:, :-1, :-1] += full_vox_occ
    ...
    full_grid_idx[1:, :-1, 1:] += full_vox_occ
    full_grid_idx[1:, 1:, 1:] += full_vox_occ
    full_grid_idx = (full_grid_idx > 0).to(torch.int32)
    sparse_grid_idx = torch.nonzero(full_grid_idx)
    full_grid_idx = torch.full([grid_res[0]+1, grid_res[1]+1, grid_res[2]+1], -1, device=space_edge.device, dtype=torch.int32)
    ...
    print("sparse_grid_idx", sparse_grid_idx.shape)

    xyz = space_min[None, ...] + sparse_grid_idx.to(torch.float32) * opt.grid_vx_sz
    # print("xyz", xyz.shape, torch.min(xyz.view(-1,3), dim=0), torch.max(xyz.view(-1,3), dim=0))
    # print("sparse_grid_idx", sparse_grid_idx.shape, sparse_grid_idx)
    # print("full_grid_idx", full_grid_idx[70:75, 70:75, 20:35], grid_res, torch.max(full_grid_idx))
    return xyz, full_grid_idx, full_vox_occ, sparse_grid_idx, grid_res, space_min, space_max
