...
class DirectContractedVoxGO(nn.Module):
    def __init__(self, xyz_min, xyz_max,
                 num_voxels=32768000, num_voxels_base=32768000,
                 alpha_init=None,
                 mask_cache_world_size=None,
                 fast_color_thres={0: 5e-06, 1500: 1e-05, 2500: 2.5e-05, 3500: 3.3333333333333335e-05, 4500: 5e-05, 5500: 0.0001, 6500: 0.0001}, bg_len=0.2,
                 contracted_norm='inf',
    ...
    def sample_ray(self, ori_rays_o, ori_rays_d, stepsize, is_train=False, **render_kwargs):
        ...
        N_inner = int(2 / (2+2*self.bg_len) * self.world_len / stepsize) + 1  # self.bg_len = 0.2， self.world_len = 320，stepsize = 0.5, N_inner = 534
        N_outer = N_inner
        b_inner = torch.linspace(0, 2, N_inner+1)  # 535
        b_outer = 2 / torch.linspace(1, 1/128, N_outer+1) # 535
        t = torch.cat([ 
            (b_inner[1:] + b_inner[:-1]) * 0.5,
            (b_outer[1:] + b_outer[:-1]) * 0.5,
        ]).cuda()   # 1068
       
        ray_pts = (rays_o[:,None,:] + rays_d[:,None,:] * t[None,:,None]).to(dtype=torch.float32)  # [8749056, 3]
        
        if self.contracted_norm == 'inf': # 1
