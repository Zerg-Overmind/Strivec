...
class PointTensorBase_dbasis(TensorBase):
    ...
    def save(self, path):
        super(PointTensorBase_dbasis, self).save(path+".th")
        info = {}
    ...
    def compute_alpha(self, xyz_locs, length=1, pnt_rmatrix=None):
        ...
        if alpha_mask.any():
            ...
            den_out, local_gindx_s, local_gindx_l, local_gweight_s, local_gweight_l, local_kernel_dist, tensoRF_id, agg_id, local_norm_xyz = self.sample_2_tensoRF_cvrg_hier(self.density_plane, self.density_line, filtered_xyz, pnt_rmatrix=pnt_rmatrix, rotgrad=False)
            sigma_feature = self.compute_densityfeature_geo(den_out, local_kernel_dist, agg_id, sample_num=len(filtered_xyz))
    ...
    def forward(self, rays_chunk, white_bg=True, is_train=False, ray_type=0, N_samples=-1, return_depth=0,
        ...
        den_out, local_gindx_s, local_gindx_l, local_gweight_s, local_gweight_l, local_kernel_dist, tensoRF_id, agg_id, local_norm_xyz = self.sample_2_tensoRF_cvrg_hier(self.density_plane, self.density_line, xyz_sampled, pnt_rmatrix=pnt_rmatrix, rotgrad=rot_step)
        ...
        sigma_feature = self.compute_densityfeature_geo(den_out, local_kernel_dist, agg_id, sample_num=len(ray_id))
        if shift is None:
        ...
        if mask.any() and (~mask).any():
            ...
            for l in range(self.lvl):
                ...
                # print("local_norm_xyz[l]", local_norm_xyz[l].shape)
...
class PointTensor_DBaseVMGS(PointTensorBase_dbasis):
    ...
    def compute_densityfeature_geo(self, den_out, local_kernel_dist, agg_id, sample_num=None):
        # plane + line basis
        sigma_feature_acc = torch.zeros([sample_num], device=den_out[0].device, dtype=torch.float32)
        num_lvl_exist = torch.zeros([sample_num, 1], device="cuda", dtype=torch.float32)
        for l in range(self.lvl):
            if len(den_out[l]) > 0:
                sigma_feature =  torch.sum(den_out[l], dim=1, keepdim=True)
                sigma_feature, has_tensorf = self.agg_tensoRF_at_samples(local_kernel_dist[l], agg_id[l], sigma_feature, out=torch.zeros([sample_num, 1], device=den_out[l].device, dtype=torch.float32), outweight=torch.zeros([sample_num, 1], device=den_out[l].device, dtype=torch.float32))
        ...
        for l in range(self.lvl):
            if len(local_gindx_s[l]) > 0:
                if self.args.vm_agg_mode == 1:
                    ...
                    plane_out, line_out = GridSample1dVm_winds.apply(self.app_plane[l], self.app_line[l * 3], self.app_line[l * 3 + 1], self.app_line[l * 3 + 2], local_gindx_s[l], local_gindx_l[l], local_gweight_s[l], local_gweight_l[l], tensoRF_id[l])
                    coef_point = torch.sum(plane_out * line_out, axis=-1)
                else:
    ...
    # @torch.no_grad()
    def sample_2_tensoRF_cvrg_hier(self, planes, lines, xyz_sampled, pnt_rmatrix=None, rotgrad=False):
        out_lst, local_gindx_s_lst, local_gindx_l_lst, local_gweight_s_lst, local_gweight_l_lst, local_kernel_dist_lst, tensoRF_id_lst, agg_id_lst, local_norm_xyz_lst = [], [], [], [], [], [], [], [], []
        for l in range(self.lvl):
            mask = None
            # print("tensoRF_topindx", len(self.tensoRF_topindx), self.tensoRF_topindx[l].shape)
            if self.args.tensoRF_shape == "cube":
                plane_out, line_out, local_gindx_s, local_gindx_l, local_gweight_s, local_gweight_l, local_kernel_dist, tensoRF_id, agg_id, local_norm_xyz = GridSample1dVm.apply(planes[l], lines[l*3], lines[l*3 + 1], lines[l*3 + 2], xyz_sampled.contiguous(), self.aabb[0], self.aabb[1], self.units, self.lvl_units[l], self.local_range[l], self.local_dims[l,:3], self.tensoRF_cvrg_inds[l], self.tensoRF_count[l], self.tensoRF_topindx[l], self.geo_xyz[l], self.K_tensoRF[l], self.KNN)
            elif self.args.tensoRF_shape == "sphere":
                print("no implementation")
                exit()
            ...
            out_lst.append(torch.sum(plane_out * line_out, dim=-1))
        ...
        return out_lst, local_gindx_s_lst, local_gindx_l_lst, local_gweight_s_lst, local_gweight_l_lst, local_kernel_dist_lst, tensoRF_id_lst, agg_id_lst, local_norm_xyz_lst
