...
std::vector<torch::Tensor> sample_pts_on_rays_geo_cuda(
        torch::Tensor rays_o, torch::Tensor rays_d, torch::Tensor pnt_xyz, torch::Tensor tensoRF_per_ray,
...
torch::Tensor raw2alpha_randstep_backward_cuda(torch::Tensor exp, torch::Tensor grad_back, torch::Tensor interval);
...
std::vector<torch::Tensor> sample_pts_on_rays_geo(
        torch::Tensor rays_o, torch::Tensor rays_d, torch::Tensor pnt_xyz, torch::Tensor tensoRF_per_ray,
        torch::Tensor half_range_sqr,
        torch::Tensor xyz_min, torch::Tensor xyz_max,
        const float near, const float far, const float stepdist,
        torch::Tensor local_range, torch::Tensor local_dims, torch::Tensor local_stepsize) {
  CHECK_INPUT(rays_o);
  CHECK_INPUT(rays_d);
  CHECK_INPUT(pnt_xyz);
  CHECK_INPUT(tensoRF_per_ray);
  CHECK_INPUT(xyz_min);
  CHECK_INPUT(xyz_max);
  assert(rays_o.dim()==2);
  assert(rays_o.size(1)==3);
  return sample_pts_on_rays_geo_cuda(rays_o, rays_d, pnt_xyz, tensoRF_per_ray, half_range_sqr, xyz_min, xyz_max, near, far, stepdist, local_range, local_dims, local_stepsize);
}


std::vector<torch::Tensor> sample_pts_on_rays_dist(
        torch::Tensor rays_o, torch::Tensor rays_d,
        torch::Tensor xyz_min, torch::Tensor xyz_max,
        const float near, const float far, const float stepdist, torch::Tensor shift) {
  CHECK_INPUT(rays_o);
  ...
  assert(rays_o.dim()==2);
  assert(rays_o.size(1)==3);
  return sample_pts_on_rays_dist_cuda(rays_o, rays_d, xyz_min, xyz_max, near, far, stepdist, shift);
}
...
std::vector<torch::Tensor> raw2alpha_randstep(torch::Tensor density, const float shift, torch::Tensor interval) {
  CHECK_INPUT(density);
  CHECK_INPUT(interval);
  assert(density.dim()==1);
  return raw2alpha_randstep_cuda(density, shift, interval);
}

torch::Tensor raw2alpha_randstep_backward(torch::Tensor exp, torch::Tensor grad_back, torch::Tensor interval) {
  CHECK_INPUT(exp);
  CHECK_INPUT(interval);
  CHECK_INPUT(grad_back);
  return raw2alpha_randstep_backward_cuda(exp, grad_back, interval);
}
...
torch::Tensor filter_ray_by_points(torch::Tensor xyz, torch::Tensor pnts,  torch::Tensor half_range) {
  CHECK_INPUT(xyz);
  CHECK_INPUT(pnts);
  assert(xyz.dim()==3);
  assert(pnts.dim()==2);
  return filter_ray_by_points_cuda(xyz, pnts, half_range);
}


torch::Tensor filter_ray_by_projection(torch::Tensor rays_o, torch::Tensor rays_d, torch::Tensor pnts, torch::Tensor half_range_sqr) {
  CHECK_INPUT(rays_o);
  CHECK_INPUT(rays_d);
  CHECK_INPUT(pnts);
  assert(rays_o.dim()==2);
  assert(rays_d.dim()==2);
  assert(pnts.dim()==2);
  return filter_ray_by_projection_cuda(rays_o, rays_d, pnts, half_range_sqr);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("sample_pts_on_rays_dist", &sample_pts_on_rays_dist, "Sample points on rays with shading dist");
  m.def("sample_pts_on_rays_geo", &sample_pts_on_rays_geo, "Sample points on rays with point tensoRF");
  ...
  m.def("raw2alpha_backward", &raw2alpha_backward, "Backward pass of the raw to alpha");  m.def("raw2alpha_randstep", &raw2alpha_randstep, "Raw values [-inf, inf] to alpha [0, 1].  if the step is not the same");
  m.def("raw2alpha_randstep_backward", &raw2alpha_randstep_backward, "Backward pass of the raw to alpha if the step is not the same");
  ...
  m.def("filter_ray_by_points", &filter_ray_by_points, "filter ray if nearby points");
  m.def("filter_ray_by_projection", &filter_ray_by_projection, "filter ray if within projection");
}
